#training a model on only CDR3s

#get the folder in which the training data is stored
import sys
...
text_datasets = {
    "train": [f'{data_folder}/train_cdr3.txt'],
    "eval": [f'{data_folder}/val_cdr3.txt'],
    "test": [f'{data_folder}/test_cdr3.txt']
}

dataset = load_dataset("text", data_files=text_datasets, cache_dir="cluster/scratch/wglaenzer/huggingface_cache")
...
antiberta_config = {
    ...
    "batch_size": 96,
    "max_steps": 22500, #originally 225000
    "weight_decay": 0.01,
...
args = TrainingArguments(
    ...
    max_steps=22500, #originally 225000
    save_steps=250,#originally 2500
    logging_steps=250, #originally 2500
    adam_beta2=0.98,
    adam_epsilon=1e-6,
    weight_decay=0.01,
    warmup_steps=10000,
...
trainer.train()
trainer.save_model(f'trained_model_normal_size_hundred_million{data_folder}')
...
ax.set_title('Loss')
plt.show()
fig.savefig('loss_steps_cdr3.pdf')
...
plt.show()
fig.savefig('eval_loss_steps_cdr3.pdf')
