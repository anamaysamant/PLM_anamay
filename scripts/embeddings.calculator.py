...
init_list = [ProtBert,Sapiens]
suffixes = ["protbert","sapiens"]
cellranger_paths = []
...
for sample in os.listdir(data_folder_path):
    ...
    for i,model in enumerate(init_list):
        ...
        save_filepath = os.path.join(save_path,f"embeddings_{suffixes[i]}.csv")
        ...
        embeds.to_csv(save_filepath, index=False)